...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--dry_run", action="store_true")   # 1 update sanity run
...
def main():
    ...
    t_days = 8 if args.dry_run else None
    gcfg = globalsetting(
        seed=args.seed,
        device=device,
        N_ASSETS=5,  # adjust if needed or add arg
        T_days=(t_days if t_days is not None else globalsetting().T_days),
