...
from GBMEnv import GBMBandEnvMulti,globalsetting
...
def _stage1_only_beta_obs(o: torch.Tensor, sigmas_np: np.ndarray) -> torch.Tensor:
    ...
    # global: [lam, mean_sigma, port_var, ||Rw||] = [1, mean(sigmas), 0, 0]
    glob = torch.tensor(
        [1.0, float(np.mean(sigmas_np)), 0.0, 0.0], dtype=torch.float32, device=device_
    ).view(1, 4).expand(B, 4)
...
def rollout_joint(policy: JointBandPolicy,
    ...
    for _ in range(be):
        ...
        env  = make_env(gcfg, R)
        obs  = env.reset(beta=beta, lam=lam, w0=None)
