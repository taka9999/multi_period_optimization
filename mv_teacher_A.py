...
def mv_teacher_single(
    ...
    *,
    ...
    """Convenience wrapper for a single instance teacher."""
    Cov_b = np.asarray(Cov, float)[None, :, :]
    beta_b = np.asarray(beta, float)[None, :]
    tgt_b = np.array([float(target_annual)], dtype=float)
    out = mv_teacher_batch(
        Cov_batch=Cov_b,
        beta_batch=beta_b,
        sigmas=sigmas,
        r=r,
        target_annual_batch=tgt_b,
        allow_cash=allow_cash,
        return_info=return_info,
    )
    if return_info:
        W, info = out
        return W[0], {k: (v[0] if hasattr(v, "__len__") and len(v)==1 else v) for k,v in info.items()}
    else:
        return out[0]
