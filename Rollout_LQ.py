...
from GBMEnv_LQ import GBMBandEnvMulti,globalsetting
...
def _stage1_only_beta_obs(o: torch.Tensor, sigmas_np: np.ndarray, target_ret_dt: float) -> torch.Tensor:
    """
    Stage-1 用に観測を β のみに差し替える。
    形式は [N*5 + 4] を保ちつつ、w=0, (R@w)=0, lam=1, port_var=0 等のダミーを詰める。
    """
    B, D = o.shape
    N = (D - 4) // 5
    device_ = o.device
    ...
    # global: [lam, target_ret_dt, port_var, ||Rw||] = [1, target, 0, 0]
    glob = torch.tensor(
        [1.0, float(target_ret_dt), 0.0, 0.0], dtype=torch.float32, device=device_
    ).view(1, 4).expand(B, 4)

    return torch.cat([per_f, glob], dim=1)         # [B, N*5+4]
...
def rollout_joint(policy: JointBandPolicy,
    ...
    for _ in range(be):
        ...
        for t in range(env.T):
            ...
            # Stage-1 はβのみ
            # Stage-1 は β のみに差し替え（ただし target_ret_dt は残す）
            o_stage = _stage1_only_beta_obs(o, gcfg.sigmas, env.target_ret_dt) if stage == 1 else o
            ...
            if stage == 1:
                ...
                # ★ 修正：Stage-2は m を決定論、s だけサンプル
                m_t, s_t, _, m_pre, s_pre = policy.sample_stage2(o_stage)
