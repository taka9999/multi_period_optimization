...
class globalsetting:
    ...
    MV_LAMBDA: float = 1.0          # risk aversion in quadratic utility
    TARGET_RET_ANN: float = 0.05    # default annual target (if reset target_ret is None)
...
class GBMBandEnvMulti:
    ...
    def step(self, A: np.ndarray, B: np.ndarray, *, use_trade_penalty: bool=True):
        ...
        # LQ / MV-style per-step utility around the target
        mv_lambda = float(self.cfg.MV_LAMBDA)
        u_mv = r_simple - 0.5 * mv_lambda * (r_simple**2)
