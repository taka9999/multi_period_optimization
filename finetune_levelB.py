...
from Rollout_HJB_box_v2 import rollout_joint_levelB, rollout_eval_levelB
from GBMEnv_LQ_v3 import globalsetting, GBMBandEnvMulti
...
def make_fixed_eval_cases_old(
    N, *, base_seed=777, n_cases=32,
    beta_mode="mixed", beta_fixed=None,
    ...
    for k in range(n_cases):
        ...
        if beta_mode == "mixed":
            ...
            beta = np.asarray(beta_fixed, float).reshape(N,)
        lam = float(rng.choice(lam_choices))
        target = float(rng.choice(target_choices))
...
def finetune_stage2_levelB(
    ...
    before = eval_levelB_fixed_cases(
        gcfg, R, policy, fixed_cases, device=gcfg.device,
        mv_solver=mv_solver, qp_solver=qp_solver,
        mv_allow_cash=mv_allow_cash
    )
    print("[Before FT fixed B-eval]", before)

    # (optional) fine-tune 前のB評価
    #eval0 = rollout_eval_levelB(policy, cfg, gcfg=gcfg, lam_choices=lam_choices, target_choices=target_choices,
    #                            batch_episodes=16, R=R, mv_allow_cash=mv_allow_cash, mv_solver=mv_solver, qp_solver=qp_solver)
    #print("[Before FT] B-eval:", eval0)
    ...
    for k in range(1, fine_tune_epochs + 1):
        ...
        #    qp_solver=qp_solver
        #)
        evalk = eval_levelB_fixed_cases(
            gcfg, R, policy, fixed_cases, device=gcfg.device,
            mv_solver=mv_solver, qp_solver=qp_solver,
            mv_allow_cash=mv_allow_cash,
            infeasible_policy="fallback",
        )
...
if __name__ == "__main__":
    ...
    # Corr matrix (same as your training setup)
    ...
    policy.load_state_dict(torch.load("policy_stage2_A2.pt", map_location=gcfg.device))
    valuef.load_state_dict(torch.load("value_stage2_A2.pt", map_location=gcfg.device))
    ...
    torch.save(policy.state_dict(), "policy_stage2_B_finetuned_log.pt")
    torch.save(valuef.state_dict(), "value_stage2_B_finetuned_log.pt")