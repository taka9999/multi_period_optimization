...
class globalsetting:
    ...
    target_annual_return: float = 0.08      # 例: 8%/year
    target_penalty_eta: float = 0        # 0なら無効。まずは 0.1 〜 10 で探索

    def __post_init__(self):
        self.T_days = int(self.years / self.dt_day)
...
class GBMBandEnvMulti:
    def __init__(self,
        ...
        if Cov is not None:
            ...
            self.R = Cov / np.outer(self.sigmas, self.sigmas)
            self.R[(np.arange(N), np.arange(N))] = 1.0
        else:
            assert sigmas is not None and R is not None
        ...
        self.target_annual_return = float(self.cfg.target_annual_return)
    ...
    def reset(self,
        ...
        if target_annual_return is not None:
            self.target_annual_return = float(target_annual_return)
    ...
    def _make_obs(self):
        ...
        global features: global features: [lam, mean_sigma, port_var, ||R w||, target_scaled]
        """
        ...
        mean_sigma = float(np.mean(self.sigmas))
        rw_norm = float(np.linalg.norm(Rw))
        # target のスケール：年率(0~0.2くらい想定)をそのまま渡してもOKだが、0~1に寄せるなら /0.2 等
        target_scaled = float(self.target_annual_return) / 0.20
        global_feats = np.array([lam_scalar, mean_sigma, port_var, rw_norm, target_scaled], float)
    ...
    def step(self, A: np.ndarray, B: np.ndarray, *, use_trade_penalty: bool=True):
        ...
        lret = math.log(max(Y_next, 1e-30)) - math.log(max(Y_prev, 1e-30))
        r_step = lret - band_pen - trade_pen

        # ---- NEW: target return penalty（最小実装）----
        # 年率ターゲット → 1ステップ(log)ターゲット（概算）
        if self.cfg.target_penalty_eta and self.cfg.target_penalty_eta > 0.0:
            steps_per_year = int(round(1.0 / self.dt))
            tgt_step = math.log(1.0 + self.target_annual_return) / max(1, steps_per_year)
            r_step = r_step - float(self.cfg.target_penalty_eta) * (lret - tgt_step)**2
